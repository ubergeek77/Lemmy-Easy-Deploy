...
**All deployment files will be placed in the `./live` directory relative to `deploy.sh`** This folder will contain a Docker Compose stack with a **stack name** of `lemmy-easy-deploy`. This deployment is not "locked into" Lemmy-Easy-Deploy in any way. You can use manage this deployment manually if you wish, **but all autogenerated files will be replaced by Lemmy-Easy-Deploy upon re-deployments**. The `.env` files in this directory contain **secrets.** *Do not delete these!*
...
If you need to specify a certain environment variable for a given service in this deployment, you can define them in any of the below files, and they will be passed to each respective service as an `env_file`. This allows you to specify any environment variable you want, for any service you want. These files follow the [Docker environment file syntax](https://docs.docker.com/compose/environment-variables/env-file/) (basically just `VAR=VAL`)
